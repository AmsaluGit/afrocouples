...
use App\Form\Filter\LanguagesFilterType;
...
class LanguagesController extends AbstractController
{
     ...
     */
    public function index(LanguagesRepository $languagesRepository, PaginatorInterface $paginator, Request $request): Response
    {

        $pageSize = 5;

$language = new Languages();
$searchForm = $this->createForm(LanguagesFilterType::class, $language);


$key = "";
if(    $request->query->get('languages_filter')    )
{
    $key = $request->query->get('languages_filter')['name'];
}

$queryBuilder = $languagesRepository->filterLanguages($key);
