...
use App\Form\Filter\MaritalStatusFilterType;
...
class MaritalStatusController extends AbstractController
{
     ...
     */
    public function index(MaritalStatusRepository $maritalStatusRepository, PaginatorInterface $paginator, Request $request): Response
    {

        $pageSize = 5;

$maritalStatus = new MaritalStatus();
$searchForm = $this->createForm(MaritalStatusFilterType::class, $maritalStatus);


$key = "";
if(    $request->query->get('maritalStatus_filter')    )
{
    $key = $request->query->get('maritalStatus_filter')['name'];
}

$queryBuilder = $maritalStatusRepository->filterByName($key);
